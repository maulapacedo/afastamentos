...
tags$body(tags$style(HTML('* {font-family: "Montserrat", sans-serif};')))
...
server <- function(input, output, session){
  ...
  output$downloadData <- downloadHandler(
    filename = function() {
      paste("meus_dados_1-", Sys.Date(), ".csv", sep = ";")
    },
    content = function(file) {
      write.csv(meus_dados_1(), file)
    }
  )
  ...
  output$summary <- renderUI({
    ...
    sumProfile <- summarytools::view(summarytools::dfSummary(meus_dados_1(),
                                                             style = "grid",
                                                             method = "render"),
                                     omit.headings = TRUE, 
                                     bootstrap.css = FALSE, 
